...
function drawTextMarkers(options) {
	...
	// Font styling
	options.ctx.font = 'bold 15px sans-serif';
	options.ctx.textBaseline = 'top';

	options.ctx.beginPath();

	// Tick every 18 (small ticks)
	for (iTick = 0; iTick <= 180; iTick += 18) {
		...
		if (iTick <= 10) {
			...
			options.ctx.fillText(iTickToPrint, (options.center.X - gaugeOptions.radius - 12) + innerTickX - 5,
					(gaugeOptions.center.Y - gaugeOptions.radius - 12) + innerTickY + 2);
		} else if (iTick < 90) {
			...
			options.ctx.fillText(iTickToPrint, (options.center.X - gaugeOptions.radius - 12) + innerTickX + 2,
					(gaugeOptions.center.Y - gaugeOptions.radius - 12) + innerTickY-1);
		} else {
			options.ctx.fillText(iTickToPrint, (options.center.X - gaugeOptions.radius - 12) + innerTickX + 11,
					(gaugeOptions.center.Y - gaugeOptions.radius - 12) + innerTickY -3);
		}

		// MPH increase by 10 every 18 degrees
		iTickToPrint += Math.round(10);
	}
...
function convertSpeedToAngle(options) {
	...
	*/
	var iSpeed = (options.speed / 10),
	    iSpeedAsAngle = ((iSpeed * 18)+2.5) % 180;
...
function drawMix(data) {
  ...
  percentGreen = Math.round(data.percent_green);
  ...
  console.log('percentGreen' + percentGreen);
  drawWithInputValue();
}
...
function drawWithInputValue() {
	...
	if (txtSpeed !== null && txtSpeed.value === 0) {

        iTargetSpeed = percentGreen;
	iTargetSpeed = Math.round(iTargetSpeed * 19/20);
        ...
        } else if (txtSpeed.value !== 0) {
		iTargetSpeed = Math.round(txtSpeed.value * 19/20);
	}

	console.log('Input Target: ' + iTargetSpeed);
        job = setTimeout("draw()", 5);
