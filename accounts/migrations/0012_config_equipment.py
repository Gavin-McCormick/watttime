...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for up in orm.UserProfile.objects.all():
            old_val = up.equipment
            if old_val:
                xs = [int(x) - 1 for x in old_val.split(',')]
            else:
                xs = []
            new_val = ','.join(str(x) for x in xs)
            up.equipment = new_val
            up.save()

    def backwards(self, orm):
        "Write your backwards methods here."
        for up in orm.UserProfile.objects.all():
            new_val = up.equipment
            if new_val:
                xs = [int(x) + 1 for x in new_val.split(',')]
            else:
                xs = []
            old_val = ','.join(str(x) for x in xs)
            up.equipment = old_val
            up.save()
